...
class AuthService {
    ...
    async register(data) {
        ...
        // Hash password
        const hashedPassword = await (0, bcryptjs_1.hash)(password, security_constants_1.BCRYPT_ROUNDS);
        ...
        // Create session
        await this.securityService.createSession(user.id, user.companyId, tokens.refreshToken, data.ipAddress, data.userAgent);
        // Track activity
        await this.securityService.trackActivity(user.id, user.companyId, 'USER_REGISTERED', { username: user.username }, data.ipAddress, data.userAgent);
        return {
            ...tokens,
            user: {
                id: user.id,
    ...
    async login(data) {
        ...
        if (companyIdentifier && !companyId) {
            ...
            if (!company) {
                throw new AppError_1.AppError('Invalid company identifier', 401);
            }
        ...
        if (isLocked) {
            throw new AppError_1.AppError('Account is locked due to too many failed attempts. Please try again later.', 401);
        }
        // Verify password
        ...
        // Track activity
        await this.securityService.trackActivity(user.id, user.companyId, 'USER_LOGGED_IN', { username: user.username }, ipAddress, userAgent);
        return {
            ...tokens,
            user: {
                id: user.id,
    ...
    async refreshToken(data) {
        ...
        try {
            ...
            const decoded = (0, jsonwebtoken_1.verify)(refreshToken, environment_1.default.JWT_REFRESH_SECRET);
            // Find user
            ...
            await this.securityService.createSession(user.id, user.companyId, tokens.refreshToken, ipAddress, userAgent);
            // Track activity
            await this.securityService.trackActivity(user.id, 'TOKEN_REFRESHED', { username: user.username }, ipAddress, userAgent);
            return {
                ...tokens,
                user: {
                    id: user.id,
    ...
    async initiateTOTPLogin(data) {
        ...
        try {
            ...
            if (!user.twoFactorEnabled || !user.twoFactorSecret) {
                ...
                const tokens = this.generateTokens(user);
                await this.securityService.createSession(user.id, user.companyId, tokens.refreshToken, ipAddress, userAgent);
                await this.securityService.trackActivity(user.id, 'USER_LOGIN', { username: user.username }, ipAddress, userAgent);
                return {
                    requiresTOTP: false,
                    ...tokens,
                    user: {
            ...
            await this.securityService.trackActivity(user.id, 'TOTP_LOGIN_INITIATED', { username: user.username }, ipAddress, userAgent);
            return {
                requiresTOTP: true,
                loginToken,
            ...
            logger_1.default.error('Error during TOTP login initiation:', error);
            throw new AppError_1.AppError('Authentication failed', 500);
        }
    }
    async verifyTOTPLogin(data) {
        ...
        try {
            ...
            if (!session) {
                throw new AppError_1.AppError('Invalid or expired login token', 401);
            }
            // Validate company context if provided
            if (companyId && session.companyId !== companyId) {
                throw new AppError_1.AppError('Company context mismatch', 401);
            }
            // Find user with tenant isolation
            const user = await this.prisma.user.findFirst({
                where: {
            ...
            if (!user || !user.twoFactorSecret) {
                throw new AppError_1.AppError('User not found or 2FA not enabled', 401);
            }
            // Verify TOTP code
            ...
            if (!verified) {
                throw new AppError_1.AppError('Invalid TOTP code', 401);
            }
            const authenticatedUser = user;
            // Generate tokens
            const tokens = this.generateTokens(authenticatedUser);
            // Create session
            await this.securityService.createSession(authenticatedUser.id, authenticatedUser.companyId, tokens.refreshToken, ipAddress, userAgent);
            await this.securityService.trackActivity(authenticatedUser.id, 'TOTP_LOGIN_SUCCESS', { username: authenticatedUser.username }, ipAddress, userAgent);
            return {
                ...tokens,
                user: {
            ...
            logger_1.default.error('Error during TOTP verification:', error);
            throw new AppError_1.AppError('TOTP verification failed', 500);
        }
    }
    async setup2FA(userId) {
        try {
            ...
            if (user.twoFactorEnabled) {
                throw new AppError_1.AppError('2FA is already enabled', 400);
            }
            // Generate secret
            ...
            await this.securityService.trackActivity(userId, '2FA_SETUP_INITIATED', { email: user.email }, undefined, undefined);
            return {
            ...
            logger_1.default.error('Error during 2FA setup:', error);
            throw new AppError_1.AppError('Failed to setup 2FA', 500);
        }
    }
    async enable2FA(userId, totpCode) {
        try {
            ...
            if (!user.twoFactorSecret) {
                throw new AppError_1.AppError('2FA setup not completed. Please run setup first.', 400);
            }
            // Verify TOTP code
            const verified = speakeasy.totp.verify({
                secret: user.twoFactorSecret,
            ...
            await this.securityService.trackActivity(userId, '2FA_ENABLED', { email: user.email }, undefined, undefined);
            ...
            logger_1.default.error('Error enabling 2FA:', error);
            throw new AppError_1.AppError('Failed to enable 2FA', 500);
        }
    }
    async disable2FA(userId, totpCode) {
        try {
            ...
            if (!user.twoFactorEnabled) {
                throw new AppError_1.AppError('2FA is not enabled', 400);
            }
            if (!user.twoFactorSecret) {
                throw new AppError_1.AppError('2FA secret not found', 400);
            }
            // Verify TOTP code
            const verified = speakeasy.totp.verify({
                secret: user.twoFactorSecret,
            ...
            await this.securityService.trackActivity(userId, '2FA_DISABLED', { email: user.email }, undefined, undefined);
            return {
                success: true,
            };
        }
            ...
            throw new AppError_1.AppError('Failed to disable 2FA', 500);
        }
    }
    async get2FAStatus(userId) {
        try {
            ...
            throw new AppError_1.AppError('Failed to get 2FA status', 500);
        }
    }
    // ===================================================================
     ...
     */
    async validateCompanyIdentifier(data) {
        try {
            ...
            logger_1.default.error('Error validating company identifier:', error);
            throw new AppError_1.AppError('Failed to validate company identifier', 500);
        }
    }
    /**
    ...
    async validateUsername(data) {
        try {
            ...
            logger_1.default.error('Error validating username:', error);
            throw new AppError_1.AppError('Failed to validate username', 500);
        }
    }
    /**
    ...
    async validatePassword(data) {
        try {
            ...
            const loginToken = crypto.randomBytes(32).toString('hex');
            await this.securityService.trackActivity(user.id, 'PASSWORD_VALIDATED_AWAITING_TOTP', { username: user.username, companyIdentifier }, undefined, undefined);
            return {
                valid: true,
                requiresTOTP: true,
                loginToken,
            ...
            logger_1.default.error('Error validating password:', error);
            throw new AppError_1.AppError('Failed to validate password', 500);
        }
    }
    /**
     * Complete TOTP Login (Step 4) - Reuses existing verifyTOTPLogin
     */
    async completeTOTPLogin(data) {
     ...
     */
    async registerSinglePage(data) {
        try {
            const { companyFullName, companyShortName, companyWorkPhone, companyCity, companyCountry, fullName, jobTitle, phoneNumber, email, username, password, } = data;
            ...
            await this.securityService.trackActivity(user.id, company.id, 'USER_REGISTERED_SINGLE_PAGE', {
                username: user.username,
                companyIdentifier,
            ...
            logger_1.default.error('Error during single-page registration:', error);
    ...
    async force2FASetup(userId) {
        try {
            ...
            await this.securityService.trackActivity(userId, 'FORCED_2FA_SETUP_INITIATED', { email: user.email }, undefined, undefined);
            return {
                secret: secret.base32,
                qrCodeImage,
                backupCodes,
            ...
            logger_1.default.error('Error during forced 2FA setup:', error);
            throw new AppError_1.AppError('Failed to setup 2FA', 500);
        }
    }
    // Helper methods for login session management
    async storeLoginSession(token, session) {
